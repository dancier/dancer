...
export CLOUD_API_TOKEN=$(cat "./cloud-api-token")
export PRIVATE_KEY_FILE="dancier.key"

IP=$(./create_next_server.py)
...
scp -i $PRIVATE_KEY_FILE -oStrictHostKeyChecking=no  ./init-instance.sh root@"${IP}":/root/
